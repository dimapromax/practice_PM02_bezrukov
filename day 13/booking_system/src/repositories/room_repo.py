...
class RoomRepository(BaseRepository[Room]):
    ...
    def get_by_number(self, hotel_id: int, number: str) -> Optional[Room]:
        """Найти номер в отеле по номеру комнаты"""
        for room in self._storage.values():
            if room.hotel_id == hotel_id and room.number == number:
                return room
        return None
